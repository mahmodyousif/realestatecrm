            ...
            <aside class="sidebar">
                ...
                <nav class="nav-links">
                    ...
                    @can('manager')
                        ...
                        <a href="{{ route('projects') }}" class="{{ request()->routeIs('projects') ? 'active' : '' }}"><i
                                class="fa-solid fa-trowel-bricks icon"></i> <span>المشاريع</span></a>
                    ...
                    @can('payments')
                        <a href="{{ route('reports') }}" class="{{ request()->routeIs('reports') ? 'active' : '' }}"><i
                                class="fa-solid fa-file-invoice-dollar icon"></i> <span>التقارير</span></a>
                        <a href="{{ route('payments') }}" class="{{ request()->routeIs('payments') ? 'active' : '' }}"><i
                                class="fa-solid fa-credit-card icon"></i> <span>إدارة الدفعات</span></a>
                    @endcan
                </nav>
            </aside>
                    ...
                    <div class="user-actions" style="display: flex; align-items: center; gap: 15px; flex-wrap: wrap;">
                        ...
                        @auth
                            <div class="user-profile"
                                style="background: var(--bg-card); padding: 5px 15px; border-radius: 12px; border: 1px solid var(--border-color); display: flex; align-items: center; gap: 10px;">
                                <span
                                    style="font-size: 14px; font-weight: 600; white-space: nowrap;">{{ auth()->user()->name }}</span>
                                <form method="POST" action="{{ route('logout') }}" style="display: inline; margin: 0;">
                                    @csrf
                                    <button type="submit"
                                        style="border: none; background: none; color: #ff5b5b; cursor: pointer; padding: 5px; min-height: auto;"
                                        title="تسجيل الخروج"><i class="fa-solid fa-power-off"></i></button>
                                </form>
                            </div>
                        @endauth
                    </div>
